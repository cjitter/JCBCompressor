      ...
      scMeterR([&p](){ return p.getSCValue(1); }) // SC meter R
{
    ...
    if (hostInfo.isLogic()) {
        ...
        titleText = "JCBCompressor v1.0.0-alpha.2";  // Nombre completo para otros DAWs
    }
...
juce::String JCBCompressorAudioProcessorEditor::getTooltipText(const juce::String& key)
{
    ...
    {
        // Spanish tooltips
        if (key == "title") return JUCE_UTF8("JCBCompressor: compresor de audio v1.0.0-alpha.2\nPlugin educativo open source\nClick para créditos");
    ...
    {
        // English tooltips
        if (key == "title") return "JCBCompressor: audio compressor v1.0.0-alpha.2\nOpen source educational plugin\nClick for credits";
