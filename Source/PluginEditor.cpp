      ...
      scMeterR([&p](){ return p.getSCValue(1); }) // SC meter R
{
    ...
    if (hostInfo.isLogic()) {
        titleText = "v0.9.99 beta";  // Solo versión para Logic Pro
    } else {
        titleText = "JCBCompressor v0.9.99 beta";  // Nombre completo para otros DAWs
    }
...
juce::String JCBCompressorAudioProcessorEditor::getTooltipText(const juce::String& key)
{
    ...
    {
        // Spanish tooltips
        if (key == "title") return JUCE_UTF8("JCBCompressor: compresor de audio v0.9.99 beta\nPlugin educativo open source\nClick para créditos");
    ...
    {
        // English tooltips
        if (key == "title") return "JCBCompressor: audio compressor v0.9.99 beta\nOpen source educational plugin\nClick for credits";
